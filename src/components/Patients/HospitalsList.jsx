...
const HospitalsList = () => {
  ...
  const fetchHospitals = async () => {
    ...
    try {
      const hospitalLists = await contract.methods.getAuthorizedHospitals().call({ from: currentAccount });
  ...
  const handleRevokeAccess = async (hospitalAddress) => {
    ...
    try {
      await contract.methods.revokeAccess(hospitalAddress).send({ from: currentAccount });
        ...
        <tbody>
          {hospitalLists.map((address, index) => (
            <tr key={index}>
              <td>{address}</td>
              <td>
                <button onClick={() => handleRevokeAccess(address)}>Revoke Access</button>
              </td>
            </tr>
          ))}
        </tbody>
