...
const Userform=()=>{
  ...
  async function handleSubmit(e) {
    ...
    const imageCid = await Promise.all(imageFiles.map(file => client.put([file])));
    ...
    const response = await fetch(`https://${jsonDataCid}.ipfs.dweb.link/data.json`);
    if (!response.ok) {
    ...
    // Decrypt the data.json content
    console.log("Encryption Key:", encryptionKey); // Log the encryption key
    
    const decryptedData = decryptData(encryptedDataJson, encryptionKey);
